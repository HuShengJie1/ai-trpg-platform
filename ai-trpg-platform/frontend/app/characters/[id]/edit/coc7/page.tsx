...
type EditCoc7CharacterPageProps = {
  params: {
    id: string;
  };
};
...
): InitialFormData<Coc7CharacterCreate> {
  return {
...
export default function EditCoc7CharacterPage({
  params,
}: EditCoc7CharacterPageProps) {
  ...
  useEffect(() => {
    ...
    getCharacter(characterId)
      .then((nextCharacter) => {
        if (normalizeRuleSystem(nextCharacter.rule_system) !== "coc7") {
  ...
  const handleSubmit = async (data: Coc7CharacterCreate) => {
    await updateCoc7Character(characterId, data);
